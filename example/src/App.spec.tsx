...
describe("Test <App />", () => {
  ...
  test("renders title within a link", async () => {
    ...
    expect(title?.closest("a")).toHaveAttribute("href", "https://www.spacex.com");
  });

  test("renders list with a no ships", async () => {
    environment.mock.resolveMostRecentOperation((operation) => {
      const res = MockPayloadGenerator.generate(operation) as GraphQLResponseWithData<Mutable<AppQueryResponse>>;
      res.data.ships = [];
      return res;
    });
    await waitForSuspense();
